...
**Playwright**: installed in `apps/web` (`@playwright/test`, config at `apps/web/playwright.config.ts`), running against a production build (`pnpm build && pnpm start`) on a dedicated port (4321) rather than `next dev` on 3000 — dev-mode's on-first-hit route compilation and a habitually-occupied port 3000 in this environment both produced false failures during setup (a redirect that hadn't happened yet looked identical to a broken gate). **Local QA only, deliberately not wired into CI** — run it yourself with `npx playwright test` (from `apps/web`); no `e2e` GitHub Actions job exists. HTML report + trace-on-first-retry available via `npx playwright show-report` after a local run.
